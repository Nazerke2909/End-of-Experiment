                ...
                ((Game) Gdx.app.getApplicationListener()).setScreen(
                    new CombatScreen(combatManager, new CombatScreen.CombatEndListener() {
                        ...
                        public void onVictory(int reward) {
                            System.out.println("Victory! You earned " + reward + " pills. Going to LevelTransitionScreen!");
                            int nextLevel = levelNumber + 1;
                            Hero[] heroesForTransition = heroes;
                            LevelData levelDataForTransition = levelData;
                            ((Game) Gdx.app.getApplicationListener()).setScreen(
                                new LevelTransitionScreen(
                                    new LevelTransitionScreen.TransitionListener() {
                                        @Override
                                        public void onNextLevel() {
                                            System.out.println("Moving to next level: " + nextLevel);
                                            navListener.goToLevel(nextLevel);
                                        }

                                        @Override
                                        public void onReturnToHub() {
                                            System.out.println("Returning to Hub from transition screen");
                                            ((Game) Gdx.app.getApplicationListener()).setScreen(GameScreen.this);
                                        }
                                    },
                                    levelDataForTransition,
                                    heroesForTransition
                                )
                            );
                        }
